...
namespace LibraryService.Services
{
    public class UserService : IUserService
    {
        ...
        public IResponse<UserCreateDto> Create(UserCreateDto user)
        {
            if (user == null)
            {
            ...
            // gereken DTO'yu entity'e dönüştür.
            var newUser = new User()
            {
                ...
                Email = user.Email,
                Password = user.Password // şifreyi hashlemeden kaydediyoruz
            };
        ...
        public IResponse<string> Login(UserLoginDto user)
        {
            ...
            var checkUser = _userRepository.GetAll().FirstOrDefault(x => (x.Username == user.Username || x.Email == user.Email) && x.Password == HashPassword(user.Password));
        ...
        private string HashPassword(string password)
        {
            ...
            string secretKey = "@~6]V@!Gh%/'kGKb/TCvuF_$$</:oq";
